...
function plot_monthly_cost_development(
    monthlyClientImbalanceCosts, 
    monthlyClientCVaRCosts, 
    clients, 
    allocations,
    simulation_months;
    save_path="Results/monthly_cost_development.png"
)
    """
    Plot the monthly cost development for clients over the simulation period.
    
    Parameters:
    - monthlyClientImbalanceCosts: Vector of cost dictionaries for each month
    - monthlyClientCVaRCosts: Vector of CVaR cost dictionaries for each month  
    - clients: Vector of client names
    - allocations: Vector of allocation methods
    - simulation_months: Number of months simulated
    - save_path: Path to save the plot
    """
    
    # Create month labels
    month_labels = ["Month $i" for i in 1:simulation_months]
    
    # Create subplots for each allocation method
    plots_list = []
    
    for allocation in allocations
        # Extract costs for this allocation method across months
        total_costs_by_month = []
        
        for month in 1:simulation_months
            month_costs = Dict()
            for client in clients
                # Combine imbalance costs and CVaR costs
                imbalance_cost = get(monthlyClientImbalanceCosts[month], (allocation, client), 0.0)
                cvar_cost = get(monthlyClientCVaRCosts[month], (allocation, client), 0.0)
                month_costs[client] = imbalance_cost + cvar_cost
            end
            push!(total_costs_by_month, month_costs)
        end
        
        # Create plot for this allocation method
        p = plot(title="Monthly Costs - $allocation", 
                xlabel="Month", 
                ylabel="Cost (EUR)",
                legend=:topright)
        ...
        for client in clients
            client_costs = [total_costs_by_month[month][client] for month in 1:simulation_months]
            plot!(p, 1:simulation_months, client_costs, 
                  label="Client $client", 
                  marker=:circle,
                  linewidth=2)
        end
    ...
    # Combine all allocation plots
    if length(plots_list) == 1
        final_plot = plots_list[1]
    else
        final_plot = plot(plots_list..., layout=(length(allocations), 1), size=(800, 600*length(allocations)))
    end
    
    # Save the plot
    savefig(final_plot, save_path)
    println("Monthly cost development plot saved to: $save_path")
