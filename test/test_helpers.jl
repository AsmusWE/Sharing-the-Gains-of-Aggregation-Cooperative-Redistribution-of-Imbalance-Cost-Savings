...
"""
    create_synthetic_systemData(clients; T=8, seed=42) -> Dict{String, Any}

Create a small synthetic systemData dict for testing.
...
- price_prod_demand_df: DataFrame with HourUTC_datetime, per-client demand,
  SolarMWh, PVForecast, ImbalanceSpreadEUR, DominatingDirection, SpotPriceEUR
- clientPVOwnership: Dict mapping client names to PV ownership fractions
"""
function create_synthetic_systemData(clients; T=8, seed=42)
    Random.seed!(seed)
    ...
    # Create dominating direction based on spread sign
    dominating_direction = ifelse.(imbalance_spread .> 0, 1, ifelse.(imbalance_spread .< 0, -1, 0))
    ...
    clientPVOwnership = Dict(client => 0.1 for client in clients)
...
"""
    create_synthetic_stochasticData(systemData, clients; num_scenarios=5, seed=42) -> Dict{String, Any}

Create synthetic stochasticData for testing.
"""
function create_synthetic_stochasticData(systemData, clients; num_scenarios=5, seed=42)
    Random.seed!(seed)
    
    T = nrow(systemData["price_prod_demand_df"])
    
    # Create demand scenarios for each client
    demand_scenarios = Dict{String, Matrix{Float64}}()
    for client in clients
        base_demand = systemData["price_prod_demand_df"][:, Symbol(client)]
    ...
    base_spread = systemData["price_prod_demand_df"][:, :ImbalanceSpreadEUR]
    ...
    base_spot = systemData["price_prod_demand_df"][:, :SpotPriceEUR]
    ...
    dominantDirection01 = ifelse.(imbalance_spread .> 0, 1, 0)
    
    stochasticData = Dict(
        "demand_scenarios" => demand_scenarios,
        "imbalance_spread" => imbalance_spread,
        "spot_price" => spot_price,
        "dominantDirection01" => dominantDirection01,
...
"""
    create_synthetic_coalitionCosts(clients; seed=42) -> Dict{Vector{String}, Float64}
...
"""
function create_synthetic_coalitionCosts(clients; seed=42)
    Random.seed!(seed)
    ...
    coalitionCosts = Dict{Vector{String}, Float64}()
    ...
    for k in 1:length(clients)
        for combo in combinations(clients, k)
            ...
            coalitionCosts[coalition] = base_cost + perturbation
        end
    end
    
    return coalitionCosts, singleton_costs
end

"""
    create_synthetic_imbalancesDict(clients, systemData; seed=42) -> Dict{Vector{String}, Vector{Float64}}

Create synthetic imbalance dict with time series for each coalition.
"""
function create_synthetic_imbalancesDict(clients, systemData; seed=42)
    Random.seed!(seed)
    
    T = nrow(systemData["price_prod_demand_df"])
    imbalancesDict = Dict{Vector{String}, Vector{Float64}}()
    
    # Generate singleton imbalances first
    client_imbalances = Dict{String, Vector{Float64}}()
    for client in clients
        demand = systemData["price_prod_demand_df"][:, Symbol(client)]
        pv = systemData["price_prod_demand_df"][:, :SolarMWh] .* systemData["clientPVOwnership"][client]
    ...
    for k in 1:length(clients)
        for combo in combinations(clients, k)
            ...
            imbalancesDict[coalition] = imbalance
        end
    end
...
Returns: (systemData, stochasticData, clients, coalitionCosts, imbalancesDict)
"""
function create_test_fixtures(; num_clients=3, T=6, seed=42)
    clients = ["Client_$(i)" for i in 1:num_clients]
    systemData = create_synthetic_systemData(clients; T=T, seed=seed)
    stochasticData = create_synthetic_stochasticData(systemData, clients; num_scenarios=3, seed=seed+1)
    coalitionCosts, _ = create_synthetic_coalitionCosts(clients; seed=seed+2)
    imbalancesDict = create_synthetic_imbalancesDict(clients, systemData; seed=seed+3)
    
    return systemData, stochasticData, clients, coalitionCosts, imbalancesDict
end
